...
public static class NodeMetadataBuilder
{
    public static NodeMetadata BuildFrom(Node node)
    {
        ...
        var flowOutputs = type.GetFieldsByType(typeof(IFlow)).ToList();
        var valueInputs = type.GetFieldsByType(typeof(IValueInput)).ToList();
        var valueOutputs = type.GetFieldsByType(typeof(IValueOutput)).ToList();
