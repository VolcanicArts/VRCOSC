...
{
    protected override async Task<Result<T>> TryTransformValueAsync(string? path, PulseContext c)
    {
    ...
    protected abstract Task<T> HandlePathAsync(string path, PulseContext c);
}
...
{
    protected override Task<T> HandlePathAsync(string path, PulseContext c) => func(path);
}

public abstract class HandleFilePathTransformNode<T>(string outputName = "") : HandleFilePathTransformAsyncNode<T>(outputName)
{
    protected override Task<T> HandlePathAsync(string path, PulseContext c) => Task.FromResult(HandlePath(path, c));

    protected abstract T HandlePath(string path, PulseContext c);
}
...
{
    protected override T HandlePath(string path, PulseContext c) => func(path);
}

public abstract class TryHandleFilePathActionAsyncNode(string pathInputName = "") : TryActionAsyncNode
{
    public ValueInput<string?> Path = new(pathInputName);
    ...
    protected abstract Task<bool> TryHandlePathAsync(string path, PulseContext c);
}

public abstract class TryHandleFilePathActionNode(string pathInputName = "") : TryHandleFilePathActionAsyncNode(pathInputName)
{
    protected override Task<bool> TryHandlePathAsync(string path, PulseContext c) => Task.FromResult(TryHandlePath(path, c));

    protected abstract bool TryHandlePath(string path, PulseContext c);
}