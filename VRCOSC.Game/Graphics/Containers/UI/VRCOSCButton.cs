...
public class VRCOSCButton : osu.Framework.Graphics.UserInterface.Button
{
    ...
    {
        if (Enabled.Value) this.ScaleTo(scale_hovered, 100, Easing.OutCirc);
        return true;
    }

    protected override void OnHoverLost(HoverLostEvent e)
    {
        if (Enabled.Value) this.ScaleTo(scale_default, 100, Easing.OutCirc);
    }

    protected override bool OnClick(ClickEvent e)
    {
        if (Enabled.Value) this.ScaleTo(IsHovered ? scale_hovered : scale_default, 500, Easing.OutElastic);
        return base.OnClick(e);
    }

    protected override bool OnMouseDown(MouseDownEvent e)
    {
        if (Enabled.Value) this.ScaleTo(scale_mousedown, 1000, Easing.OutSine);
        return true;
    }
}
